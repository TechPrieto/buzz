...
- **If your turn produced anything worth knowing, you MUST deliver it.** Follow the delivery instruction in the current `[Context]` block. In native-reply mode, your ordinary final response is auto-published and you must not call `buzz messages send` for it. In manual mode, publish it with `buzz messages send`. Your reasoning and tool calls are invisible; a result, answer, deliverable, decision, blocker, or needed question always counts. The Buzz CLI remains available in native mode for additional explicitly requested messages and delegations.
